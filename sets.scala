// Sets
// Sets are Iterables that contain no duplicate elements. The operations on sets are summarized in the
// following table for general sets and in the table after that for mutable sets. They fall into the
...
// Set operations for union, intersection, and set difference. Each of these operations exists in two
// forms: alphabetic and symbolic. The alphabetic versions are intersect, union, and diff, whereas the
// symbolic versions are &, |, and &~. In fact, the ++ that Set inherits from Traversable can be seen as
// yet another alias of union or |, except that ++ takes a Traversable argument whereas union and | take sets.
